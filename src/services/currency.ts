...
export class Currency {
    async getRates() {
        ...
        const rates = await this.fetchURL(
            'https://api.belo.app/public/price',
            options
        );
    ...
    async fetchURL(url = '', options = {}) {
        ...
        while (isLoading && retries <= MAX_RETRIES) {
            try {
                ...
                retries++;
                console.log(
                    "Belo API didn't respond. Attempt number:",
                    retries
                );
    ...
    convertPrice(
        productPrice: number,
        USDTPriceInARS: number,
        decimals: number = 2
    ) {
        return `${(productPrice / USDTPriceInARS).toFixed(decimals)}`;
    }
}
